...
temp := random(2)+1;
  ...
  begin
    ...
    '又来我这里存宝物了,最近我发现打到宝物的勇士越来越多了\我这里都快存不下了。如果您也想多存点宝物的话，\就去问问罗家老板，他或许知道点线索。\'
    +//'|{cmd}<开启仓库/@DoInputDialog1>\'
    +'|{cmd}<仓库管理/@storage>\'
    //+'|{cmd}<找回东西/@getback>\'
    +'|{cmd}<捆各种卷书和药水/@mbind>\'
    +'|{cmd}<打听“挑战”平局的消息/@tiaozhan>\'
    +'|{cmd}<退出/@doexit>'
  );
  end
  else
...
begin
  This_Npc.NpcDialog(This_Player,
  '高手较量，胜负往往只在一瞬间。\我也偶有眼神不济之时，不敢贸然断定胜负，故会判为“平局”。\当然，你们所押的宝物也可各自收回。\如果你有足够的信心，可以再次挑战对手。\相信对你来说，再赢一次没有什么困难吧？\|{cmd}<返回/@main>'
  );
end;
