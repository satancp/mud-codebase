...
begin
  ...
  +'|{cmd}<买戒指/@buy>'+ addSpace('', 24) + '|{cmd}<卖戒指/@sell>\'
  +'|{cmd}<修理戒指/@repair>\'
  +'|{cmd}<特殊修理/@s_repair>\'
...
begin
  This_Npc.NpcDialog(This_Player,
  '由于我本领有限，升级成强化物品，需要3个原有的首饰才可以，\也就是说你想要升级成强化雷霆战戒，就必须给我3个雷霆战戒，\除了3个首饰，还需要提供给我一些配方，否则，我也无能为力，\首先，升级需要888颗金刚石，可以保证升级后物品的纯度，\升级除了相应的绒丝，还需要魔龙颚骨、魔龙骨、魔龙牙各一件，\如果你有上述物品，你就交给我吧，我保证给您一个强化的首饰，\您想升级什么物品呢？就把它们交给我吧。\'
  +'|{cmd}<升级雷霆战戒/@uplei>'+ addSpace('', 8) + '|{cmd}<升级烈焰魔戒/@upmo>'+ addSpace('', 8) + '|{cmd}<升级光芒道戒/@upgua>\'
  +'|{cmd}<返回/@main>' 
  );
end;
...
procedure _uplei;
begin
  DoUpgrade('雷霆战戒', '雷霆绒丝', '魔龙颚骨', '魔龙骨', '魔龙牙', '强化雷霆战戒');
end;

procedure _upmo;
begin
  DoUpgrade('烈焰魔戒', '烈焰绒丝', '魔龙颚骨', '魔龙骨', '魔龙牙', '强化烈焰魔戒');    
end;

procedure _upgua;
begin
  DoUpgrade('光芒道戒', '光芒绒丝', '魔龙颚骨', '魔龙骨', '魔龙牙', '强化光芒道戒');  
end;
