...
begin
  ...
  '恭喜你进入了<30号/c=red>奖励藏宝阁，在完成我的要求之后，\你还可以得到额外的奖励。请记得在包裹中留下至少五格空间，\'
  +'否则将得不到奖励。\5个乾坤令可以捆绑成一个乾坤令包。\'
...
begin
  ...
  begin
    ...
    begin
      This_Npc.NpcDialog(This_Player,
      '恭喜你，你将获得乾坤令包' + inttostr(dianshu-1) +'个，\请在包裹中保留足够的空间。\'
      +'你投出的骰子的点数为：'+IntToStr(dianshu) +'\你可以前进'+IntToStr(dianshu) +'个房间\'
      +'|{cmd}<移动/@next_cube>');
    end;   
  end;
...
begin
  if This_Player.GetS(12,4) <> 300 then
  begin
    This_Player.SetS(12,4,300);  
    case dice of
    1:
      begin
        This_Player.Give('热血积分',1);
        This_Npc.NpcNotice('恭喜：' + This_Player.Name + '，在藏宝阁30号获得：热血积分1个！');
      end;
    2,3,4,5,6:
      begin
        This_Player.Give('乾坤令包',dice-1);
        This_Npc.NpcNotice('恭喜：' + This_Player.Name + '，在藏宝阁30号获得：乾坤令包' + inttostr(dice-1) +'个！'); 
      end;
    end;
  end;  
end;
...
begin
  ...
  This_Player.RandomFlyTo('EM002~31');
  if diceNum > 1 then
  begin
    prizegive(diceNum);   //调用奖励过程     
    This_Player.CallOut(This_Npc,1,'addroom');
...
procedure _leave;
begin
  if compareText(This_Player.MapName, 'EM002~30') = 0 then
  begin
