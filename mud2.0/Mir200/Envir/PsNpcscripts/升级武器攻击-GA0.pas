...
begin
  ...
  +'<祖玛以上武器/c=red>\'
  +'升级每次需要10元宝：\'
  +'<可以升级武器攻魔道，最高+7/c=red>\'

  +'|{cmd}<开始升级武器攻击/@Checkup_1> \'
...
begin
     ...
     begin
       ...
       begin                             
         ...
         if (ck_num > 0) and (ck_num < 7) then     //检测是不是小于7  小于继续可以升级 
         begin
     ...
     if ck_num > 6 then  //这里检测是不是大于6也就是=7，大于6就无法继续升级 
     ...
     begin                                                                                             
       ...
       begin
         ...
         This_Player.PsYBConsum(This_NPC,'ForgeDiaYB',20199,10,1);      //扣10元宝 
