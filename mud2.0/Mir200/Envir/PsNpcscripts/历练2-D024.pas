...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if This_NPC.CheckMapMonByName('D024','恶魔沃玛') > 80 then
        begin
          This_Npc.NpcDialog(This_Player,          
          '目前已经放出够多的恶魔沃玛，请先消灭完，\如果你在我附近没有看到恶魔沃玛 你可以试着跑远点看看\');
          This_Player.SetS(22,24,220);
          This_Player.DecGold(3300);
        end
        else
        begin
          This_NPC.CreateMon('D024',62,62,5,'恶魔沃玛',10);
          This_Player.SetS(22,24,220);
          This_Player.DecGold(3300);
          This_NPC.CloseDialog(This_Player); 
        end;
...
begin
  ...
  begin
    ...
    begin
      This_Npc.NpcDialog(This_Player,          
      '这位勇士，你果然厉害，已经杀满10只恶魔沃玛，\'
      +'我这就给你超额的奖励。奖励内容为：<经验 600000/c=red> \ \');
    ...
    begin
      This_Npc.NpcDialog(This_Player,          
      '历练的方法很简单，只要你给我<3300金币/c=red>，\'
      +'我将会在我所站的周围，释放<10只恶魔沃玛/c=red>，当你打死他们后，\'
      +'我就会给你<超额的奖励/c=red>。但如果你没有<打满10只怪/c=red>，\'
      +'可再交付我<3300金币/c=red>。\'
      +'我将再释放<10只恶魔沃玛/c=red>。直到你打满10只怪为止。\'
      +'奖励内容为：<经验 600000/c=red>\'
      +'|{cmd}<支付金币/@giveqian>');
    end;
  end
  else
