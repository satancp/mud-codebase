...
begin
  This_Npc.NpcDialog(This_Player,
  '你已经了解了此处名胜，可以考虑去姻缘神殿看看了。\难得你有兴趣听了我这么多唠叨，这点东西算是一点心意了。\ \'
...
var
  data1, iRan, iTemnum, iFins, item, iTemn02, iTemExp: integer;
  dNow, dNow02 : double;
begin
  dNow := GetNow();
  ...
  begin
    if This_Player.GetS(17,6) = 4 then
    begin
       This_Npc.NpcDialog(This_Player,
       '你已经了解了此处名胜，可以考虑去姻缘神殿看看了。');
       Exit;
    end else if This_Player.GetS(17,6) <> 3 then
    begin
       This_Npc.NpcDialog(This_Player,
    ...
    begin
       ...
       begin
          ...
          iFins := This_Player.GetS(17,8);
          dNow02 := ConvertDBToDateTime(iFins);                      //将保存的整型再还原为浮点数double；
          item := minusDataTime(dNow,dNow02);                        //计算两个时间点之间的秒数之差；
          ...
          begin
             ...
             begin
                This_Player.Give('巡游手记第四章',1);
                This_Npc.NpcDialog(This_Player,
                '你已经了解了此处名胜，可以考虑去姻缘神殿看看了。');
             end;
             
             This_Player.PlayerNotice('你没有在5分钟内找到旅行家。',2);
             This_Player.CallOut(This_NPC,1,'RightFly04');
             //This_Player.Flyto('M101',12,35); 
          end else
          begin
             if iRan <= 9999 then
             begin
                iTemExp := iTemnum * 16000;
                iTemExp := iTemExp + 160000;
             ...
             This_Player.PlayerNotice('恭喜你在5分钟内找到旅行家！',2);
             This_Player.LeftTime('恭喜你在5分钟内找到旅行家：',20001,1);
             This_Player.CallOut(This_NPC,1,'RightFly04');
             //This_Player.Flyto('M101',12,35); 
          end;
       end;
...
//一秒后自动传送 
procedure RightFly04; 
begin
  if CompareText(This_Player.MapName, 'DM002') = 0 then
  begin
     This_Player.Flyto('M101',12,35);
  end;
end;
...
begin
   ...
   if (iTod = This_Player.GetS(17,5)) and ((This_Player.GetS(17,6) = 3) or (This_Player.GetS(17,6) = 4)) then
   begin
      This_Npc.NpcDialog(This_Player,
      '国王陵墓是上古某一位比奇国王死后安葬的地方。\它隐秘在洞穴之中，\'
      +'僵尸和骷髅据说都是自愿为他守灵的军队死后的灵魂，\'
      +'经过层层历险进入陵墓的人。\难道这还不足以向世人展现它的风采？\ \'
