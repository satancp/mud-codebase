...
begin
  ...
  '恭喜你进入了<10号/c=red>奖励藏宝阁，在完成我的要求之后，\你还可以得到额外的奖励。请记得在包裹中留下至少五格空间，\'
...
begin
  if This_Player.GetS(12,4) = 0 then
  begin
    This_Player.SetS(12,4,100);  
    case dice of
    1:
      begin
        This_Player.Give('热血积分',1);
        This_Npc.NpcNotice('恭喜：' + This_Player.Name + '，在藏宝阁10号获得：热血积分1个！');
      end;
    2,3,4,5,6:
      begin
        This_Player.Give('乾坤令包',dice-1);
        This_Npc.NpcNotice('恭喜：' + This_Player.Name + '，在藏宝阁10号获得：乾坤令包' + inttostr(dice-1) +'个！'); 
      end;
    end;
  end;  
end;
...
begin
  ...
  This_Player.RandomFlyTo('EM002~11');
  if diceNum > 1 then
  begin
...
procedure _leave;
begin
  if compareText(This_Player.MapName, 'EM002~10') = 0 then
  begin
