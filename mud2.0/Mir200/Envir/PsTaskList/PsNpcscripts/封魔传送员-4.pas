...
begin 
    ...
    This_Player.Flyto('D2000',30 + random(3) - 1,175 + random(3) - 1); 
end; 
 
procedure domain; 
begin 
    This_NPC.NpcDialog(This_Player, 
    '这里通向满是野猪的封魔矿区，如果你在历练的过程中遇到<虹魔教主/c=red>，\请千万注意自己，它们是矿区中让人无法安心的梦魇。\ \'  
    +'|{cmd}<前往封魔矿区/@goToMonMap>\' 
    +'|{cmd}<前往疾风殿/@GoldToMonMap_1>        \'//^<霸者大厅/@GoldToMonMap_2>
    +'|{cmd}<购买物品/@buy>            ^<出售物品/@sell>\' 
    +ActiveValidateStr
    ); 
end;

procedure _GoldToMonMap_1;
begin
    This_Npc.NpcDialog(This_Player,  
    '我可以送你前往疾风殿，不过必须首先给我2万金币。\ \'
    +'|{cmd}<前往疾风殿/@GoldToMonMap_1_1>\ \'
    +'|{cmd}<返回/@main>' 
  );
end;

procedure _GoldToMonMap_1_1;
begin
