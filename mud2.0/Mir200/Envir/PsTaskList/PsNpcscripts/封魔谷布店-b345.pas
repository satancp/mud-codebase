...
Begin
   ...
   begin
      This_Npc.NpcDialog(This_Player,
      '最近发生的一连串妖魔的事件，一定有古怪，听说妖魔是\'+
      '另有目的的。我怀疑我们中间出了内奸，一定要想办法把那人\'+
      '给找出来，线索就在他给妖魔的那些藏宝图上。\ \'+
      '|{cmd}<老板，你的店里布料怎么样啊？/@duihua1>\'+
      '|{cmd}<打听天工神剪的故事/@gongdian>\'+
      '|{cmd}<买衣服/@buy>'+addspace(' ',26)+'|{cmd}<卖衣服/@sell>\'+
      '|{cmd}<修复衣服/@repair>'+addspace(' ',24)+'|{cmd}<特殊修理/@s_repair>\'+
      '|{cmd}<退出/@doexit>');
   end;
   if j = 0 then 
   begin
      This_Npc.NpcDialog(This_Player,
      '我这里的布料可是应有尽有哦，要什么有什么！\ \'+
      '|{cmd}<老板，你的店里布料怎么样啊？/@duihua1>\'+
      '|{cmd}<打听天工神剪的故事/@gongdian>\'+
      '|{cmd}<买衣服/@buy>'+addspace(' ',26)+'|{cmd}<卖衣服/@sell>\'+
      '|{cmd}<修复衣服/@repair>'+addspace(' ',24)+'|{cmd}<特殊修理/@s_repair>\'+
