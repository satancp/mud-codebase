...
begin
  This_Npc.NpcDialog(This_Player,
  '灰色毒粉的原料有 <食人树的果子/@helpitem_1> 2个，\毒蜘蛛牙齿 4个,\食人树叶 8个。\ \|{cmd}<返回/@helpmakedrug>'
  );
end;
...
begin
  This_Npc.NpcDialog(This_Player,
  '黄色毒粉的原料有 <蛆卵/@helpitem_2> 2个, 蝎子的尾巴\4个, 食人树叶 8个。\ \|{cmd}<返回/@helpmakedrug>'
  );
end;
...
begin
  This_Npc.NpcDialog(This_Player,
  '切就食人草可以得到食人树的果子，\但是5个食人草中可以得到果实的，\只有1个。\ \|{cmd}<返回/@helpmakedrug1>'
  );
end;
...
begin
    if (This_Player.GetBagItemCount('薄荷叶') >= 1) and (This_Player.GetV(11,7) = 3) then
    begin
